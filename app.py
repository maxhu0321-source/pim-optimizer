...
""")
    ...
    with st.expander("1. 酒店特色信息（PiM第1页）", expanded=False):
        st.markdown("""
- **酒店描述**：中英文各一段，英文描述会显示在希尔顿官网
- **位置短描述**：≤38个字符（含空格），简要说明地理位置
- **经纬度**：注意纬度（18-54）在前，经度（73-135）在后，不要填反
- **座机电话**：必填，不能填占位符（如123456）
- **ADR（平均房价）**：必须填写合理数值
- **到达信息**：详细描述从机场/火车站如何到达酒店
- **城市与距离**：填写距主要地标的距离，需含具体数字+单位（KM）
""")

    with st.expander("2. 房型房价信息（PiM第2页）", expanded=False):
        st.markdown("""
- **房型代码**：必须与PMS代码表一致（如 SKR、SVR、SLS 等）
- **房量**：每个房型的房间数，PiM和PMS必须完全一致
- **面积**：填整数（平方米），不能填小数或区间（如"25-30"）
- **无烟房**：选 Yes/No，Yes 表示该房型全部为无烟房
- **最大入住人数**：欢朋默认为 3 人
- **无障碍房**：至少1个房型需标注为无障碍房
- **连通房**：品牌标准要求至少 2 间
...
""")
    ...
    with st.expander("5. 客房设施信息（PiM第5页）", expanded=False):
        st.markdown("""
- **无烟房数量**：必须与第2页的无烟房数据一致
- **沙发/沙发床**：数量不能超过对应房型总房间数
- **连通房**：数量必须与第2页一致
- **浴袍**：数量不能超过总房间数
- **SLS（亲子套房）**：如有此房型，必须配沙发床
- **折叠床**：需要填写数量（可以为0）
""")
...
| # | 错误 | 频率 | 说明 |
|---|------|------|------|
| 1 | 房型代码PiM/PMS不一致 | 🔴 极高 | 两表房型代码必须完全相同 |
| 2 | 同一房型房量不一致 | 🔴 极高 | PiM和PMS的房间数必须一致 |
| 3 | 无烟房数量前后矛盾 | 🔴 高 | 第2页和第5页的无烟房数据不一致 |
